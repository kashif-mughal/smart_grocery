...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 13, 2020 at 12:32 PM
...
INSERT INTO `grocery_category` (`Id`, `CatName`, `Alias`, `Img`, `ParentId`, `CreatedOn`, `ModifiedOn`, `Status`) VALUES
...
CREATE ALGORITHM=UNDEFINED DEFINER=`technider`@`%` SQL SECURITY DEFINER VIEW `supplier_transaction_summary`  AS  select `supplier_information`.`supplier_name` AS `supplier_name`,`supplier_ledger`.`supplier_id` AS `supplier_id`,'credit' AS `type`,sum(-`supplier_ledger`.`amount`) AS `amount` from (`supplier_ledger` join `supplier_information` on(`supplier_information`.`supplier_id` = `supplier_ledger`.`supplier_id`)) where `supplier_ledger`.`deposit_no` is null group by `supplier_ledger`.`supplier_id` union all select `supplier_information`.`supplier_name` AS `supplier_name`,`supplier_ledger`.`supplier_id` AS `supplier_id`,'debit' AS `type`,sum(`supplier_ledger`.`amount`) AS `amount` from (`supplier_ledger` join `supplier_information` on(`supplier_information`.`supplier_id` = `supplier_ledger`.`supplier_id`)) where `supplier_ledger`.`chalan_no` is null group by `supplier_ledger`.`supplier_id` ;
