...
class Returnse extends CI_Model {
	...
	public function return_invoice_entry()
	{
		...
		$rate = $this->input->post('product_rate_original');
		$p_id = $this->input->post('product_id');
		$p_uuid = $this->input->post('product_uuid');
...
///start  return list
			...
			{
				$this->db->select('SUM(a.total_ret_amount) total_amount,a.*,b.customer_name');
				$this->db->from('product_return a');
				$this->db->join('customer_information b','b.customer_id = a.customer_id');
				$this->db->where('usablity',1);
				$this->db->group_by('a.invoice_id','desc');
