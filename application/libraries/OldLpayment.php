...
class Lpayment {
	...
	public function receipt_form(){
		...
		$account_list_category=$CI->Payment->account_list_category();
        $account_list=$CI->Payment->account_list();

		

		$loan_list = $CI->Settings->loan_list();
		$data = array(
	        'title' 	=>  display('add_receipt'),
	        'supplier'	=>	$supplier,
	        'customer'	=>	$customer,
	        'bank'    	=>	$bank,
	        'account_list'=>$account_list,
	...
	public function payment_up_data($trans)
	{
		...
		$data=array(
		   	'title'  			 => display('update_transaction'),
	...
	public function payment_list_date_wise($start,$end)
	{
		$CI =& get_instance();
		$CI->load->model('Payment');
