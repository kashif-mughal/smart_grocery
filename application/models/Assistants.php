...
class Assistants extends CI_Model {
    ...
    public function search_last_assistant($key, $value) {
        $this->db->select("gp.ProductId, gp.ProductName, gp.tags, gp.Price, gp.SalePrice, gp.ProductImg, gb.BrandName, concat(gp.SaleUnitQty, gps.UnitName) SaleUnitQuantity, gc.CatName, gc2.CatName parentCategory, CASE WHEN gu.UnitName = NULL THEN 'Piece' ELSE gu.UnitName END AS UnitName ");
